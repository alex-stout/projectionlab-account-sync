...
describe("AccountList", () => {
  it("shows empty state with plugin name when no accounts", () => {
    render(<AccountList plugin={plugin} accounts={[]} mappings={{}} plAccounts={[]} sourceError={null} onMappingChange={vi.fn()} />);
  ...
  it("shows plugin hint in empty state when provided", () => {
    ...
    render(<AccountList plugin={pluginWithHint} accounts={[]} mappings={{}} plAccounts={[]} sourceError={null} onMappingChange={vi.fn()} />);
    expect(screen.getByText("Custom hint text")).toBeInTheDocument();
  });

  it("renders a row for each account", () => {
    render(<AccountList plugin={plugin} accounts={accounts} mappings={{}} plAccounts={[]} sourceError={null} onMappingChange={vi.fn()} />);
  ...
  it("shows sourceError when provided", () => {
    render(<AccountList plugin={plugin} accounts={[]} mappings={{}} plAccounts={[]} sourceError="Tab not open" onMappingChange={vi.fn()} />);
