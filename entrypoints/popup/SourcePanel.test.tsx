...
const accounts = [{ name: "IRA", balance: 5000, accountId: null }];
...
const getRefreshSourceBtn = () =>
	screen.getByRole("button", { name: /↻ Vanguard/ });
...
describe("SourcePanel", () => {
	...
	it("calls onSynced and shows results after successful sync", async () => {
		...
		render(
			<SourcePanel {...defaults} plAccounts={plAccounts} onSynced={onSynced} />,
		);
	...
	it("shows error state when sync returns error", async () => {
		...
		await waitFor(() =>
			expect(screen.getByText("PL not open")).toBeInTheDocument(),
		);
	});
	...
	it("resets error state when plugin changes", async () => {
		...
		const { rerender } = render(
			<SourcePanel {...defaults} plAccounts={plAccounts} />,
		);
		await waitFor(() => screen.getByText("Sync to ProjectionLab"));
		await act(async () => {
			fireEvent.click(screen.getByText("Sync to ProjectionLab"));
