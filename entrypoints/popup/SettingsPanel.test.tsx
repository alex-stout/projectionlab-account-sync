...
describe("SettingsPanel", () => {
	...
	it("reverts '✓ Cleared' to 'Clear All Data' after the 2s timer elapses", async () => {
		...
		try {
			...
			expect(
				screen.queryByRole("button", { name: "✓ Cleared" }),
			).not.toBeInTheDocument();
		} finally {
			vi.useRealTimers();
		}
...
describe("SettingsPanel — Plugins section", () => {
	it("renders a toggle for every plugin", () => {
		...
		expect(
			screen.getByRole("checkbox", { name: "Enable Alight" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("checkbox", { name: "Enable YNAB" }),
		).toBeInTheDocument();
	});
...
describe("SettingsPanel — ApiPluginCreds (YNAB)", () => {
		...
		vi.mocked(browser.storage.local.get).mockImplementation((key: any) => {
			if (key === "creds_ynab")
				return Promise.resolve({
					creds_ynab: { accessToken: "seeded-token" },
				} as any);
	...
	it("shows '✓ Saved' feedback after saving YNAB creds", async () => {
		...
		expect(
			screen.getAllByRole("button", { name: /✓ Saved/ }).length,
		).toBeGreaterThan(0);
	});
	...
	it("reverts YNAB '✓ Saved' to 'Save' after the 2s timer elapses", async () => {
		...
		try {
			...
			expect(screen.queryAllByRole("button", { name: /✓ Saved/ })).toHaveLength(
				0,
			);
		} finally {
			vi.useRealTimers();
	...
	it("resets '✓ Saved' feedback when the token is edited after save", async () => {
		...
		const savedCount = screen.queryAllByRole("button", {
			name: /^✓ Saved$/,
		}).length;
	...
	it("Clear All Data resets the visible YNAB token input even if creds were loaded", async () => {
		...
		vi.mocked(browser.storage.local.get).mockImplementation((key: any) => {
			if (key === "plApiKey")
				return Promise.resolve({ plApiKey: "pl-key" } as any);
			if (key === "creds_ynab")
				return Promise.resolve({
					creds_ynab: { accessToken: "tok-abc" },
				} as any);
