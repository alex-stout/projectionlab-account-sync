...
describe("App", () => {
	...
	it("marks plugin as available when a matching tab is open", async () => {
		...
		await waitFor(() => {
			...
			expect(
				btn.querySelector(".bg-indigo-200, .bg-green-400"),
			).toBeInTheDocument();
		});
	});
		...
		await waitFor(() =>
			expect(browser.storage.local.set).toHaveBeenCalledWith(
				expect.objectContaining({
					plAccounts: [{ id: "pl-1", name: "Brokerage" }],
				}),
			),
		);
		await act(async () => {
			fireEvent.click(screen.getByTitle("Vanguard"));
		});
		expect(
	...
	it("loads lastSynced from storage and shows timestamp in sidebar", async () => {
		vi.mocked(browser.storage.local.get).mockResolvedValue({
			lastSynced_vanguard: Date.now(),
		} as any);
		render(<App />);
		await waitFor(() =>
			expect(
				within(screen.getByTitle("Vanguard")).getByText("just now"),
			).toBeInTheDocument(),
		);
	});
	...
	it("loads lastRefreshed from storage and shows timestamp in panel header", async () => {
		vi.mocked(browser.storage.local.get).mockResolvedValue({
			lastRefreshed_vanguard: Date.now(),
		} as any);
		render(<App />);
		await waitFor(() =>
	...
	it("handleSynced updates sidebar timestamp after successful sync", async () => {
		...
		vi.mocked(browser.runtime.sendMessage).mockResolvedValue({
			results: [{ name: "IRA", ok: true }],
		} as any);
		render(<App />);
		await waitFor(() => screen.getByText("Sync to ProjectionLab"));
		await act(async () => {
			fireEvent.click(screen.getByText("Sync to ProjectionLab"));
		});
		await waitFor(() =>
			expect(
				within(screen.getByTitle("Vanguard")).getByText("just now"),
	...
	it("shows plError banner when FETCH_PL_ACCOUNTS returns an error", async () => {
		vi.mocked(browser.runtime.sendMessage).mockResolvedValue({
			error: "No API key set. Open extension settings.",
		} as any);
		render(<App />);
		await waitFor(() => screen.getByTitle("Settings"));
		await act(async () => {
	...
	it("saving YNAB creds from Settings flips the sidebar availability dot", async () => {
		...
		await waitFor(() =>
			expect(ynabBtn.querySelector(".bg-green-400")).toBeInTheDocument(),
		);
	});
	...
	it("Clear All Data resets plAccounts, lastSynced, and lastRefreshed in the UI", async () => {
		vi.mocked(browser.storage.local.get).mockImplementation((keys: any) => {
			...
			if (arr.includes("lastSynced_vanguard"))
				result.lastSynced_vanguard = Date.now();
	...
	it("falls back to the first enabled plugin when the active one is disabled via toggle", async () => {
		...
		await act(async () => {
			fireEvent.click(
				screen.getByRole("checkbox", { name: "Enable Vanguard" }),
			);
		});
		await waitFor(() =>
			expect(screen.queryByTitle("Vanguard")).not.toBeInTheDocument(),
		);
	});
	...
	it("silently ignores FETCH_PL_ACCOUNTS responses with neither accounts nor error", async () => {
		...
		expect(
			screen.getByRole("option", { name: "Existing" }),
		).toBeInTheDocument();
	});
