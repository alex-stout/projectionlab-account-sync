...
beforeEach(() => {
	...
	vi.mocked(browser.storage.local.get)
		.mockReset()
		.mockResolvedValue({ plApiKey: "test-key" } as any);
...
describe("SYNC_SOURCE", () => {
	...
	it("writes accounts and lastRefreshed to storage on success", async () => {
		...
		expect(browser.storage.local.set).toHaveBeenCalledWith(
			expect.objectContaining({
				accounts_vanguard: payload,
				lastRefreshed_vanguard: expect.any(Number),
			}),
		);
	});
	...
	it("propagates the tab's specific error message when ok is false", async () => {
		...
		expect(result.error).toBe(
			"No accounts found on this page. Please open an issue.",
		);
	});

	it("returns error when tabs.sendMessage throws", async () => {
		vi.mocked(browser.tabs.query).mockResolvedValueOnce([mockTab] as any);
		vi.mocked(browser.tabs.sendMessage as any).mockRejectedValue(
...
describe("SYNC_TO_PL", () => {
	...
	it("uses accountId as mapping key when present", async () => {
		...
		expect(browser.tabs.sendMessage).toHaveBeenCalledWith(
			mockTab.id,
			expect.objectContaining({
				entries: [{ plId: "pl-uuid-1", balance: 5000, name: "IRA" }],
			}),
		);
	});
