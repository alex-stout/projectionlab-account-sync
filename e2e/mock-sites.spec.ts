...
test("refreshes Vanguard accounts from mock tab", async ({
	context,
	popupBaseUrl,
}) => {
...
test("loads ProjectionLab accounts into dropdowns from mock tab", async ({
	...
	await expect(
		firstSelect.locator("option", { hasText: "Roth IRA" }),
	).toBeAttached();
	await expect(
		firstSelect.locator("option", { hasText: "Traditional 401k" }),
	).toBeAttached();
});

test("full sync: Vanguard → ProjectionLab", async ({
	context,
	popupBaseUrl,
}) => {
	await openMockTab(context, VANGUARD_URL);
...
test("refreshes Monarch accounts from mock tab", async ({
	context,
	popupBaseUrl,
}) => {
...
test("full sync: Monarch → ProjectionLab", async ({
	context,
	popupBaseUrl,
}) => {
	await openMockTab(context, MONARCH_URL);
...
test("PL accounts loaded in Settings populate dropdowns for every plugin", async ({
	...
	await seedStorage(sw, {
		accounts_vanguard: [{ name: "Roth IRA", balance: 1200, accountId: null }],
		accounts_alight: [
