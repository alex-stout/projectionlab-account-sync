...
test("Clear All Data removes the API key and all plugin data", async ({
	page,
	context,
}) => {
	const sw = await getServiceWorker(context);
...
test("disabling a plugin hides it from the sidebar and persists to storage", async ({
	...
	await expect(page.getByTitle("Alight")).toBeVisible();
	await page.getByTitle("Settings").click();
	await page.locator('label:has(input[aria-label="Enable Alight"])').click();
...
test("re-enabling a disabled plugin brings it back to the sidebar", async ({
	...
	await page.getByTitle("Settings").click();
	await page.locator('label:has(input[aria-label="Enable YNAB"])').click();
...
test("disabling all plugins forces the settings view", async ({ page }) => {
	await page.getByTitle("Settings").click();
	await page.locator('label:has(input[aria-label="Enable Vanguard"])').click();
	await page.locator('label:has(input[aria-label="Enable Alight"])').click();
	await page.locator('label:has(input[aria-label="Enable YNAB"])').click();
...
test("PL key Clear button removes the key but preserves plugin data", async ({
	...
	const seededAccounts = [{ name: "Roth IRA", balance: 1200, accountId: null }];
	await seedStorage(sw, {
		plApiKey: "test-api-key",
